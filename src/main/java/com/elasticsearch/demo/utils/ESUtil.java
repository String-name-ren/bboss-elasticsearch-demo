...
public class ESUtil {
	 ...
	 * 添加或修改文档
	 */
	public void addOrUpdateDocument(String index,String type,Object obj){
		//获取文档的客户端对象，单实例多线程安全
		ClientInterface clientUtil = ESClient.restClient();
		//向固定index添加或者修改文档,如果id已经存在做修改操作，否则做添加文档操作，返回处理结果
		//索引表 索引类型
		clientUtil.addDocument(index,type,obj);
	}
	 ...
	 * 删除文档
	 */
	public void deleteDocumentByIds(String index,String type,String[] ids){
		//获取文档的客户端对象，单实例多线程安全
		ClientInterface clientUtil = ESClient.restClient();
		...
		for(int i = 0; i < ids.length; ++i) {
			String id = ids[i];
			builder.append("{ \"delete\" : { \"_index\" : \"").append(index).append("\", \"_type\" : \"").append(type).append("\", \"_id\" : \"").append(id).append("\" } }\n");
		}
		//索引表 索引类型 id 对象
